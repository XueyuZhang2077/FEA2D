...
def Geom(geom: pygmsh.occ.Geometry, vertices: ndarray, lcar: ndarray, regions: list):
    ...
    line_vertexID= unique(sort(array(line_vertexID) ,axis=1) ,axis=0)
    ...
    for i in range(len_regions):
        ...
        for j in range(1, len(region)):
            ...
            v0_inlineI=where(line_vertexID==edge_v0)[0]
            v1_inlineI=where(line_vertexID==edge_v1)[0]
            lineID=intersect1d(v0_inlineI,v1_inlineI)[0]
...
class Mesh2D:
    def __init__(self, node_coord, cell_nodeID, domain_cellID,order=1):
        self.order=order
        ...
        self.cellArea = (abs(cross(P3 - P1, P2 - P1)) / 2)
        #############

    def savetxt(self, mesh_name: str):
        savetxt(mesh_name + "__node_coord.txt", self.node_coord)
        savetxt(mesh_name + "__cell_nodeID.txt", self.cell_nodeID)
        savetxt(mesh_name + "__domainNum.txt", array([len(self.domain_cellID)]))
        for i in range(len(self.domain_cellID)):
            savetxt(mesh_name + f"__domain{i}_cell.txt", self.domain_cellID[i])
        ...
        if self.order == 2:
            ...
            for i in range(len(self.cell_nodeID)):
                ...
                for j in range(3):
                    ...
                    d = linalg.norm(self.node_coord - new_coord, axis=1)
                    if any(d < 0.0000001):
                        cell_nodeID[i].append(where(d < 0.0000001)[0][0])
                    else:
                        self.node_coord = append(self.node_coord, array([new_coord]), axis=0)
                        cell_nodeID[i].append(len(self.node_coord) - 1)
            self.cell_nodeID = array(cell_nodeID)
            self.order = 2
...
def LargeMeshCellID_InAsubMesh(cell_node_coordFromSubMesh,cell_coordFromLargeMesh):
    ...
    cellArea_SubMesh = (abs(cross(P3 - P1, P2 - P1)) / 2)
    ...
    for i in range(cell_node_coordFromSubMesh.shape[0]):
        ...
        Area_CPiPj=abs(cross(C - P1, C - P2)) / 2+abs(cross(C - P2, C - P3)) / 2+abs(cross(C - P3, C - P1)) / 2

        err=Area_CPiPj-cellArea_SubMesh[i]

        res=append(res,where(abs(err)<1e-7)[0])

    return int_(unique(array(res)))
...
def GenerateMeshes2D(vertex_coord: ndarray, vertex_mshSize: ndarray, domain_vertexID: list) :
    print("Generating Meshes")
    with pygmsh.occ.Geometry() as geom:
        Geom(geom, vertex_coord, vertex_mshSize, domain_vertexID)
        msh = geom.generate_mesh()
    mesh=Mesh2D(msh.points,msh.cells_dict["triangle"],domain_cellID=None)
